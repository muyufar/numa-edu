<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h2 class="text-xl font-bold leading-tight text-nu-primary">{{ __('Edit berita') }}</h2>
                <p class="mt-1 text-sm text-gray-600">{{ $berita->judul }}</p>
            </div>
            <a href="{{ route('berita.index') }}" class="inline-flex items-center rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 shadow-sm hover:bg-gray-50">{{ __('Kembali') }}</a>
        </div>
    </x-slot>
    ...
    <div class="space-y-4">
        ...
        <div class="rounded-2xl border border-gray-100/80 bg-white p-5 shadow-sm ring-1 ring-black/5 sm:p-8">
            <form method="POST" action="{{ route('berita.update', $berita) }}" class="space-y-6">
                @csrf
                @method('PUT')
                @if ($errors->any())
                ...
                @endif
                @include('berita._form', ['berita' => $berita])
        ...
        @can('delete', $berita)
            <div class="rounded-2xl border border-gray-100/80 bg-white p-5 shadow-sm ring-1 ring-black/5 sm:p-8">
                <h3 class="text-sm font-bold text-gray-900">{{ __('Hapus berita') }}</h3>
                <p class="mt-1 text-xs text-gray-600">{{ __('Tindakan ini tidak dapat dibatalkan.') }}</p>
                <form method="POST" action="{{ route('berita.destroy', $berita) }}" class="mt-4" onsubmit="return confirm('{{ __('Hapus berita ini?') }}')">
                    @csrf
                    @method('DELETE')
