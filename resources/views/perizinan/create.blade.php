<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h2 class="text-xl font-bold leading-tight text-nu-primary">{{ __('Pengajuan perizinan') }}</h2>
                <p class="mt-1 text-sm text-gray-600">{{ __('Izin, sakit, atau dispensasi untuk siswa.') }}</p>
            </div>
    ...
    <div class="space-y-4">
        ...
        <div class="rounded-2xl border border-gray-100/80 bg-white p-5 shadow-sm ring-1 ring-black/5 sm:p-8">
            <form method="GET" action="{{ route('perizinan.create') }}" class="grid gap-4 sm:grid-cols-2 sm:items-end">
                <div>
                ...
                <button type="submit" class="inline-flex items-center justify-center rounded-xl bg-nu-primary px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-nu-primary-light">{{ __('Tampilkan siswa') }}</button>
            </form>
        </div>

        @if ($kelasId && $siswas->isNotEmpty())
            <div class="rounded-2xl border border-gray-100/80 bg-white p-5 shadow-sm ring-1 ring-black/5 sm:p-8">
                <form method="POST" action="{{ route('perizinan.store') }}" class="space-y-6">
                    @csrf
                    ...
                    @endif
                    @include('perizinan._form', ['perizinan' => null, 'siswas' => $siswas])
                    <div class="flex items-center justify-end border-t border-gray-100 pt-5">
                        <button type="submit" class="inline-flex items-center justify-center rounded-xl bg-nu-primary px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-nu-primary-light">{{ __('Kirim') }}</button>
                    </div>
                </form>
            </div>
        @elseif ($kelasId && $siswas->isEmpty())
            <div class="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">{{ __('Tidak ada siswa di kelas ini.') }}</div>
        @endif
    </div>
</x-app-layout>
