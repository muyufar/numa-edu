    ...
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <h2 class="text-xl font-bold leading-tight text-nu-primary">{{ $guru->nama }}</h2>
                <p class="mt-1 text-sm text-gray-600">{{ __('Guru') }}</p>
            </div>
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('guru.show', $guru) }}" class="inline-flex items-center rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 shadow-sm hover:bg-gray-50">
                    {{ __('Lihat detail') }}
                </a>
    ...
    <div class="rounded-2xl border border-gray-100/80 bg-white p-5 shadow-sm ring-1 ring-black/5 sm:p-8">
        <form method="POST" action="{{ route('guru.update', $guru) }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="flex items-center justify-end gap-3 border-t border-gray-100 pt-5">
                <a href="{{ route('guru.show', $guru) }}" class="text-sm font-semibold text-gray-600 hover:text-gray-900">
                    {{ __('Batal') }}
                </a>
