<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h2 class="text-xl font-bold leading-tight text-nu-primary">{{ __('Edit item kurikulum') }}</h2>
                <p class="mt-1 text-sm text-gray-600">{{ __('Perbarui struktur kurikulum.') }}</p>
            </div>
    ...
    <div class="rounded-2xl border border-gray-100/80 bg-white p-5 shadow-sm ring-1 ring-black/5 sm:p-8">
        <form method="POST" action="{{ route('kurikulum.update', $item) }}" class="space-y-6">
            @csrf
            @method('PUT')
            @include('kurikulum._form', ['item' => $item])
