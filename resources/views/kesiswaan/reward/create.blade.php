<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h2 class="text-xl font-bold leading-tight text-nu-primary">{{ __('Catat reward') }}</h2>
                <p class="mt-1 text-sm text-gray-600">{{ __('Pilih kelas, lalu isi data reward siswa.') }}</p>
            </div>
    ...
    <div class="space-y-4">
        ...
        <div class="rounded-2xl border border-gray-100/80 bg-white p-5 shadow-sm ring-1 ring-black/5 sm:p-8">
            <form method="GET" action="{{ route('kesiswaan.reward.create') }}" class="grid gap-4 sm:grid-cols-2 sm:items-end">
                <div>
        ...
        @if ($kelasId && $siswas->isNotEmpty())
            <div class="rounded-2xl border border-gray-100/80 bg-white p-5 shadow-sm ring-1 ring-black/5 sm:p-8">
                <form method="POST" action="{{ route('kesiswaan.reward.store') }}" class="space-y-6">
                    @csrf
                    ...
                    @endif
                    @include('kesiswaan.reward._form', ['row' => null, 'siswas' => $siswas])
