<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h2 class="text-xl font-bold leading-tight text-nu-primary">{{ __('Presensi pegawai') }}</h2>
                <p class="mt-1 text-sm text-gray-600">{{ __('Riwayat kehadiran staf non-guru.') }}</p>
            </div>
    ...
    <div class="space-y-4">
        ...
        <div class="rounded-2xl border border-gray-100/80 bg-white p-4 shadow-sm ring-1 ring-black/5 sm:p-5">
            <form method="GET" action="{{ route('presensi.pegawai.index') }}" class="flex flex-col gap-3 sm:flex-row sm:items-end">
                ...
                <div class="flex gap-2">
                    <button type="submit" class="inline-flex items-center justify-center rounded-xl bg-nu-primary px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-nu-primary-light">{{ __('Terapkan') }}</button>
                    <a href="{{ route('presensi.pegawai.index') }}" class="inline-flex items-center justify-center rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50">{{ __('Reset') }}</a>
                </div>
            </form>
        </div>
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        @forelse ($rows as $row)
                            <tr class="hover:bg-gray-50/80">
                                <td class="px-5 py-3 font-mono text-gray-900">{{ $row->tanggal?->format('Y-m-d') }}</td>
                                <td class="px-5 py-3 font-medium text-gray-900">{{ $row->pegawai?->nama ?? '—' }}</td>
                                <td class="px-5 py-3">
                                ...
                                <td class="px-5 py-3">
                                    @can('delete', $row)
                                        <form method="POST" action="{{ route('presensi.pegawai.destroy', $row) }}" class="flex justify-end" onsubmit="return confirm('{{ __('Hapus baris presensi ini?') }}')">
                                            @csrf
                                            @method('DELETE')
