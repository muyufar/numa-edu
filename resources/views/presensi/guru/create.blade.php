<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h2 class="text-xl font-bold leading-tight text-nu-primary">{{ __('Input presensi guru') }}</h2>
                <p class="mt-1 text-sm text-gray-600">{{ __('Pilih tanggal, lalu isi status untuk setiap guru.') }}</p>
            </div>
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('presensi.guru.index') }}" class="inline-flex items-center rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 shadow-sm hover:bg-gray-50">
                    {{ __('Riwayat') }}
                </a>
    ...
    <div class="space-y-4">
        ...
        <div class="rounded-2xl border border-gray-100/80 bg-white p-5 shadow-sm ring-1 ring-black/5 sm:p-8">
            <form method="GET" action="{{ route('presensi.guru.create') }}" class="flex flex-col gap-4 sm:flex-row sm:items-end">
        ...
        @if ($gurus->isNotEmpty())
            <div class="rounded-2xl border border-gray-100/80 bg-white p-5 shadow-sm ring-1 ring-black/5 sm:p-8">
                <form method="POST" action="{{ route('presensi.guru.store') }}" class="space-y-6">
                    @csrf
                        ...
                        <table class="min-w-full divide-y divide-gray-100 text-sm">
                            ...
                            <tbody class="divide-y divide-gray-100 bg-white">
                                @foreach ($gurus as $g)
                                    @php
                                        $i = $loop->index;
                                        $defStatus = $existing->get($g->id)?->status ?? 'hadir';
                                        $statusVal = old("presensi.$i.status", $defStatus);
                                        $ketVal = old("presensi.$i.keterangan", $existing->get($g->id)?->keterangan);
                                    @endphp
                                    <tr class="hover:bg-gray-50/80">
                                        <td class="px-4 py-3 font-medium text-gray-900">{{ $g->nama }}</td>
                                        <td class="px-4 py-3 font-mono text-gray-700">{{ $g->nip ?: '—' }}</td>
                                        <td class="px-4 py-3">
                                            <input type="hidden" name="presensi[{{ $i }}][guru_id]" value="{{ $g->id }}" />
                                            <select name="presensi[{{ $i }}][status]" class="w-full min-w-[8rem] rounded-lg border border-gray-200 bg-white px-2 py-2 text-sm focus:border-nu-primary focus:outline-none focus:ring-2 focus:ring-nu-primary/20">
                                                @foreach (\App\Models\PresensiGuru::STATUS_OPTIONS as $st)
        ...
        @else
            <div class="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                {{ __('Belum ada data guru. Tambahkan guru di master terlebih dahulu.') }}
            </div>
        @endif
    </div>
</x-app-layout>
