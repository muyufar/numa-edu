<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h2 class="text-xl font-bold leading-tight text-nu-primary">{{ __('Buat tagihan') }}</h2>
                <p class="mt-1 text-sm text-gray-600">{{ __('Tagihan baru berstatus belum lunas hingga ada pembayaran.') }}</p>
            </div>
    ...
    <div class="rounded-2xl border border-gray-100/80 bg-white p-5 shadow-sm ring-1 ring-black/5 sm:p-8">
        <form method="POST" action="{{ route('tagihan.store') }}" class="space-y-6">
            @csrf
            ...
            @include('tagihan._form', ['siswaOptions' => $siswaOptions])

            <div class="flex items-center justify-end gap-3 border-t border-gray-100 pt-5">
                <a href="{{ route('tagihan.index') }}" class="text-sm font-semibold text-gray-600 hover:text-gray-900">{{ __('Batal') }}</a>
