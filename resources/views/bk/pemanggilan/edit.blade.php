<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h2 class="text-xl font-bold leading-tight text-nu-primary">{{ __('Edit pemanggilan') }}</h2>
                <p class="mt-1 text-sm text-gray-600">{{ $row->siswa?->nama }} · {{ $row->tanggal_jadwal?->format('d M Y') }}</p>
            </div>
    ...
    <div class="space-y-4">
        ...
        <div class="rounded-2xl border border-gray-100/80 bg-white p-5 shadow-sm ring-1 ring-black/5 sm:p-8">
            ...
            @else
                <form method="POST" action="{{ route('bk.pemanggilan.update', $row) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    @include('bk.pemanggilan._form', ['row' => $row, 'siswas' => $siswas])
        ...
        @can('delete', $row)
            <div class="rounded-2xl border border-gray-100/80 bg-white p-5 shadow-sm ring-1 ring-black/5 sm:p-8">
                ...
                <p class="mt-1 text-xs text-gray-600">{{ __('Hapus jadwal pemanggilan permanen dari basis data.') }}</p>
                <form method="POST" action="{{ route('bk.pemanggilan.destroy', $row) }}" class="mt-4" onsubmit="return confirm('{{ __('Hapus jadwal pemanggilan ini?') }}')">
                    @csrf
                    @method('DELETE')
