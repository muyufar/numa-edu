<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="text-xl font-bold leading-tight text-nu-primary">{{ __('BK — Tambah jenis pelanggaran') }}</h2>
            <p class="mt-1 text-sm text-gray-600">{{ __('Buat master jenis pelanggaran baru.') }}</p>
        </div>
    </x-slot>

    <div class="space-y-4">
        <div class="rounded-2xl border border-gray-100/80 bg-white p-5 shadow-sm ring-1 ring-black/5">
            <form method="POST" action="{{ route('bk.jenis-pelanggaran.store') }}" class="space-y-5">
                @csrf
