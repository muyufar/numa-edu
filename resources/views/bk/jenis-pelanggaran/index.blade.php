<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h2 class="text-xl font-bold leading-tight text-nu-primary">{{ __('BK — Jenis pelanggaran') }}</h2>
                <p class="mt-1 text-sm text-gray-600">{{ __('Master jenis pelanggaran, poin, dan tingkat.') }}</p>
            </div>
            @can('create', \App\Models\BkJenisPelanggaran::class)
                <a href="{{ route('bk.jenis-pelanggaran.create') }}" class="inline-flex items-center justify-center rounded-xl bg-nu-primary px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-nu-primary-light">
                    {{ __('Tambah') }}
                </a>
            @endcan
        </div>
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        @forelse ($rows as $row)
                            <tr class="hover:bg-gray-50/80">
                                <td class="px-5 py-3 font-mono text-xs text-gray-700">{{ $row->kode }}</td>
                                <td class="px-5 py-3 font-medium text-gray-900">{{ $row->nama }}</td>
                                <td class="px-5 py-3 font-mono text-xs text-gray-700">{{ $row->poin }}</td>
                                ...
                                <td class="px-5 py-3 text-right">
                                    @can('update', $row)
                                        <a href="{{ route('bk.jenis-pelanggaran.edit', $row) }}" class="inline-flex items-center rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs font-semibold text-gray-700 hover:bg-gray-50">{{ __('Edit') }}</a>
                                    @endcan
                                </td>
                            </tr>
                        @empty
