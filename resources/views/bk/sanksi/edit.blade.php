<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h2 class="text-xl font-bold leading-tight text-nu-primary">{{ __('BK — Edit sanksi') }}</h2>
    ...
    <div class="space-y-4">
        ...
        <div class="rounded-2xl border border-gray-100/80 bg-white p-5 shadow-sm ring-1 ring-black/5">
            <form method="POST" action="{{ route('bk.sanksi.update', $row) }}" class="space-y-5">
                @csrf
                @method('PUT')
        ...
        @can('delete', $row)
            <div class="rounded-2xl border border-gray-100/80 bg-white p-5 shadow-sm ring-1 ring-black/5">
                ...
                <p class="mt-1 text-xs text-gray-600">{{ __('Hapus sanksi dari master data.') }}</p>
                <form method="POST" action="{{ route('bk.sanksi.destroy', $row) }}" class="mt-4" onsubmit="return confirm('{{ __('Hapus sanksi ini?') }}')">
                    @csrf
                    @method('DELETE')
